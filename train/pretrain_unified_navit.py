...
class TrainingArguments:
    ...
    save_mode: str = field(
        default="step",
        metadata={"help": "Save mode: 'step' for step-based saving, 'epoch' for epoch-based saving."}
    )
    save_every_epochs: float = field(
        default=1.0,
        metadata={"help": "Save a checkpoint every N epochs (only used when save_mode='epoch')."}
    )
    ...
    total_steps: Optional[int] = field(
        default=None,
        metadata={"help": "Total number of optimizer steps to train for. If None, no step limit."}
    )
    total_epochs: Optional[float] = field(
        default=None,
        metadata={"help": "Total number of epochs to train for. If None, no epoch limit."}
    )
...
def calculate_epoch_progress(global_samples_processed, dataset_meta):

    if global_samples_processed == 0:
        return 0.0

    total_dataset_samples = 0.0
    for dataset_name, cfg in dataset_meta.items():
        if "num_used_data" not in cfg or not cfg["num_used_data"]:
            raise ValueError(
                f"`num_used_data` is required for dataset '{dataset_name}' but was not found."
            )

        weight = cfg.get("weight", 1.0)
        total_dataset_samples += sum(cfg["num_used_data"]) * weight

    if total_dataset_samples <= 0:
        raise ValueError("Total dataset sample count computed as 0.")

    return global_samples_processed / total_dataset_samples
...
def main():
    ...
    logger.info(f"Dataset config file: {data_args.dataset_config_file}")
    for dataset_name, dataset_config in dataset_meta.items():
        dataset_names = dataset_config.get('dataset_names', [])
        logger.info(f"Dataset group {dataset_name}: {dataset_names}")
    ...
    current_epoch_progress = 0.0
    last_epoch_progress = 0.0
    last_saved_epoch = 0.0
    epoch_based_save = (training_args.save_mode == "epoch")
    

    for curr_step, data in enumerate(train_loader, start=train_step):
        if training_args.total_steps is not None and curr_step >= training_args.total_steps:
            if dist.get_rank() == 0:
                logger.info(f"Reached target total_steps {training_args.total_steps}, stopping training")
            break
            
        if training_args.total_epochs is not None and curr_step > 0:
            # synchronize sample count across GPUs for epoch calculation
            epoch_calc_samples_tensor = torch.tensor(total_samples_processed, device=device)
            dist.all_reduce(epoch_calc_samples_tensor, op=dist.ReduceOp.SUM)
            global_samples_for_epoch = epoch_calc_samples_tensor.item()
            
            current_epoch_progress = calculate_epoch_progress(global_samples_for_epoch, dataset_meta)
            if current_epoch_progress >= training_args.total_epochs:
                if dist.get_rank() == 0:
                    logger.info(f"Reached target total_epochs {training_args.total_epochs:.3f}, current epoch: {current_epoch_progress:.3f}. Stopping training")
                break
        ...
        if curr_step % training_args.log_every == 0:
            ...
            # synchronize global sample count across GPUs
            ...
            # compute current epoch progress if total_epochs is set
            if training_args.total_epochs is not None:
                current_epoch_progress = calculate_epoch_progress(global_total_samples, dataset_meta)
            else:
                current_epoch_progress = 0.0
            ...
            # compute sample throughput since last log
            ...
            # build log message
            if training_args.total_steps is not None:
                progress_percent = (curr_step / training_args.total_steps) * 100 if training_args.total_steps > 0 else 0
                message = f"(step={curr_step:07d}/{training_args.total_steps}) ({progress_percent:.1f}%) "
            else:
                message = f"(step={curr_step:07d}) "
                
            # append epoch progress if applicable
            if training_args.total_epochs is not None:
                message += f"(epoch={current_epoch_progress:.3f}"
                epoch_progress_percent = (current_epoch_progress / training_args.total_epochs) * 100 if training_args.total_epochs > 0 else 0
                message += f"/{training_args.total_epochs}) ({epoch_progress_percent:.1f}%) "
            else:
                message += ""
        ...
        should_save = False
        if epoch_based_save:
            # epoch-based save mode
            if curr_step > 0:

                checkpoint_epoch_samples_tensor = torch.tensor(total_samples_processed, device=device)
                dist.all_reduce(checkpoint_epoch_samples_tensor, op=dist.ReduceOp.SUM)
                checkpoint_global_samples = checkpoint_epoch_samples_tensor.item()
                
                current_epoch_progress = calculate_epoch_progress(checkpoint_global_samples, dataset_meta)
                if current_epoch_progress >= last_saved_epoch + training_args.save_every_epochs:
                    should_save = True
                    last_saved_epoch = int(current_epoch_progress / training_args.save_every_epochs) * training_args.save_every_epochs
        else:
        ...
        if should_save:
            if dist.get_rank() == 0:
                if epoch_based_save:
                    logger.info(f"💾 Saving weights at epoch {current_epoch_progress:.3f}")
                else:
                    logger.info(f"💾 Saving weights at step {curr_step}")
