...
# NEW: Argument to receive the path for the temporary JSONL output
...
def main(args):
    ...
    if torch.cuda.device_count() == 1:
        ...
        for k in same_device_modules: device_map[k] = device_map.get(k, first_device)
    checkpoint_file = "model.safetensors" if os.path.exists(os.path.join(model_path, "model.safetensors")) else "ema.safetensors"

    model = load_checkpoint_and_dispatch(
        model,
        checkpoint=os.path.join(model_path, checkpoint_file),
    ...
    # The launcher may assign per-prompt output_dir paths via metadata.
    # Report all distinct save directories for clarity instead of a single path.
    os.makedirs(args.output_dir, exist_ok=True)
    # Load metadata first to inspect their individual output directories.
    with open(args.metadata_file, "r") as f:
        metadatas = json.load(f)

    unique_dirs = sorted({m.get("output_dir", args.output_dir) for m in metadatas})
    if len(unique_dirs) == 1:
        print(f"Images will be saved in {unique_dirs[0]}")
    else:
        joined = "\n  - ".join(unique_dirs)
        print("Images will be saved in the following directories:\n  - " + joined)

    # Proceed to processing prompts (metadatas already loaded)
    ...
    for idx, metadata in enumerate(metadatas):
        ...
        save_dir = metadata.get('output_dir', args.output_dir)
        os.makedirs(save_dir, exist_ok=True)
        outpath = os.path.join(save_dir, f"{prompt_id}.png")
        ...
        if args.think:
            inference_hyper = dict(max_think_token_n=1000, do_sample=False, cfg_text_scale=4.0, cfg_img_scale=1.0, cfg_interval=[0.4, 1.0], timestep_shift=3.0, num_timesteps=50, cfg_renorm_min=0.0, cfg_renorm_type="global")
            ...
            inference_hyper = dict(cfg_text_scale=4.0, cfg_img_scale=1.0, cfg_interval=[0.4, 1.0], timestep_shift=3.0, num_timesteps=50, cfg_renorm_min=0.0, cfg_renorm_type="global")
